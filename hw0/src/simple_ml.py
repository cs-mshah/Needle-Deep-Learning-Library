...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, y.shape[0], batch):
        ...
        Iy = np.zeros((y_batch.size, theta.shape[1]))
        Iy[np.arange(y_batch.size), y_batch] = 1

        Z = np.exp(X_batch @ theta)
        sum_of_rows = Z.sum(axis=1)
        Z /= sum_of_rows[:, np.newaxis]

        theta -= (lr / y_batch.size) * (np.transpose(X_batch) @ (Z - Iy))
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, y.shape[0], batch):
        ...
        Iy = np.zeros((y_batch.size, W2.shape[1]))
        Iy[np.arange(y_batch.size), y_batch] = 1

        Z1 = ReLU(X_batch @ W1)

        G2 = np.exp(Z1 @ W2)
        sum_of_rows = G2.sum(axis=1)
        G2 /= sum_of_rows[:, np.newaxis]
        G2 -= Iy

        G1 = np.multiply(np.where(Z1 <= 0, 0, 1), G2 @ W2.T)

        W1 -= (lr / y_batch.size) * (X_batch.T @ G1)
        W2 -= (lr / y_batch.size) * (Z1.T @ G2)
